...
for i in range(nbBits):
    smiles_bit_number[0].append(i)
for i in range(nbBits):
        smiles_bit_number[1].append([''])
# Define variable to keep the smiles and bit numbers
smiles_bit_number2 = [[],[]]
for i in range(nbBits):
    smiles_bit_number2[0].append(i)
for i in range(nbBits):
    smiles_bit_number2[1].append([''])
...
for molecule_nb,m in enumerate(mols):
    ...
    if m is None:
        ...
        for i in range(0,len(items)):
            radius=items[i][1][0][1]
            atom=items[i][1][0][0]
            substructure_id = items[i][0]
            ...
            if position_track[bit] == 0: 
                ...
                if radius not in radii:
                    ...
                    smiles_bit_number[1][bit].append(ids_now[i])
                    ...
                    smiles_bit_number2[1][bit].append(str(nbFeatTot)) 
                ...
                if radius not in radii and ids_now[i]  not in smiles_bit_number[1][bit]:  
                    pass
                if radius not in radii and ids_now[i]  in smiles_bit_number[1][bit]: 
                    pass

                if radius in radii and ids_now[i]  not in smiles_bit_number[1][bit]:
                    smiles_bit_number[1][bit].append(ids_now[i] )
                    if image and ids_now[i]  not in subm_all:
                        image_name="%s_Feature_%d.pdf"%(outname,nbFeatTot)
                        Draw.MolToFile(m,image_name,size=(300,300),highlightAtoms=amap.keys())
                ...
                if submol.GetNumAtoms() >1 and ids_now[i]  in smiles_bit_number[1][bit]: 
                    fps_by_comp[0][molecule_nb].append(ids_now[i] )
    ...
    step_now=nbMols % (1+molecule_nb)
    if step_now == 0:
        step= (float(molecule_nb)/float(nbMols)) * 100
        timestr = "\rProgression: %f ... " % (step) 
...
f=open(fp_per_bit,'w')
for i in smiles_bit_number2[0]:
     if smiles_bit_number2[1][i]==['']:
         f.write("%d\n" % (i))
     else:
      f.write("%d%s\n" % (i, ','.join(list(set(smiles_bit_number2[1][i])))))
f.close()

if verbose:
    print "\nTotal number of features : %d" %(len(subm_all))
...
# Define variable to keep the smiles and bit numbers
    smiles_bit_numberEXT = [[],[[]]]
    for i in range(nbBits):
        smiles_bit_numberEXT[0].append(i)
    for i in range(nbBits):
            smiles_bit_numberEXT[1].append([''])
...
#Loop over the molecules
    for molecule_nb,m in enumerate(molsEXT):
        ...
        if m is None:
            ...
            smiles_bit_numberEXT[1][bit].append(ids_nowEXT[i]) #submol
