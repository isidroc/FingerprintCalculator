...
# Import Modules
#####################################
import argparse import ArgumentParser as _ArgumentParser
...
from rdkit.Chem import SmilesMolSupplier as _SmilesMolSupplier, MolFromMol2Block as _MolFromMol2Block, SDMolSupplier as _SDMolSupplier, MolToSmiles as _MolToSmiles, Path ToSubmol as _PathToSubmol, FindAtomEnvironmentOfTadiusN as _FindAtomEnvironmentOfRadiusN
...
parser.add_argument('--radii', '--list', nargs="+", help="Radii to be considered for the calculation of unhashed fingerprints")
...
if not fileMolsEXT:
	...
	savetxt(fp_hash_b,calc_fps_no_reference_keys.fps_hashed_binary_quick,delimiter=',')
	savetxt(fp_hash_c,calc_fps_no_reference_keys.fps_hashed_counts,delimiter=',')
	if unhashed:
		if image:
			...
			savetxt(fpbinary,calc_fps_no_reference_keys.fps_unhashed_binary,delimiter=',')
			savetxt(fpcounts,calc_fps_no_reference_keys.fps_unhashed_counts,delimiter=',')
			# save the smiles and the substructures
			fname=outname+'substructure_smiles.csv'
			savetxt(fname,calc_fps_no_reference_keys.substructures_smiles,delimiter="\t")
			# save the dictionary of substructures
			fname=outname+'substructure_dictionary.csv'
			savetxt(fname,calc_fps_no_reference_keys.substructure_dictionary,delimiter="\t")
			# save column names, i.e. to which substructure a column belongs
			fname=outname+'fps_unhashed_columns.csv'
			savetxt(fname,calc_fps_no_reference_keys.columns_unhashed,delimiter="\t")
		else:
...
# We calculate the fps w.r.t. to a reference set of molecules
	...
	#  Reference molecules
	ref_molecules=load_molecules(fileMols,name_field="_Name")
	ref_molecules.read_molecules()

# la referencia es la de filemols, no de ext
#ref_molecules_info = get_dataset_info(name_field='_Name')
	...
	reference_keys = ref_molecules_info.substructure_dictionary.keys()
	# Calculate hashed fps
	calc_fps_w_reference_keys = CalculateFPs(radii=[0,1,2],mols=test_mols.mols,reference_substructure_keys=reference_keys)
